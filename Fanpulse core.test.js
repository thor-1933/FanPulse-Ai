 ...
 * (see fanpulse-ai.html) so it can run directly as a Claude Artifact with no
...
function loadCore() {
  const htmlPath = path.join(__dirname, "fanpulse-ai.html");
  const html = fs.readFileSync(htmlPath, "utf8");

  const match = html.match(/<script id="core-logic">([\s\S]*?)<\/script>/);
  if (!match) {
    throw new Error("Could not find <script id=\"core-logic\"> block in fanpulse-ai.html");
  }
